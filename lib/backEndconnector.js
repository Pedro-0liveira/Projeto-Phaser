        ...
        success: function (response) {
			if (response!="false") {
				...
				if  (infoUser.user!="prof") {                         
					...
					// Devolvedados to fetch remaining user data
					devolveDados();
					return 1;	
				} else {
...
function devolveDados() {
	...
	if (infoUser.user != "") {
		...
		({
			...
			//console.log("Percentagem: ",percentagem); // Testing
...
saveScore = function (sc, f) {
	...
	// Session Verify commented as it's seemingly unnecessary and was Logging the user out
	//sessionVerify();
}
